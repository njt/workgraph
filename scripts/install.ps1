...
# --- 2b. detect existing install (upgrade-from) -----------------------
...
if (Test-Path $dest) {
    try {
        $oldVersion = (& $dest --version 2>&1) -replace '^wg\s+',''
        Write-Info "Upgrading from $oldVersion `u{2192} $version"
    } catch {
        Write-Info "Upgrading from unknown version `u{2192} $version"
    }
}
...
if (Test-Path $sumsPath) {
    ...
    if (-not $expected) {
        ...
        $prefix = $actual.Substring(0, 12)
        Write-Ok "SHA256 verified: $prefix ($asset)"
    }
} else {
